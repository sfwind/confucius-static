...
export default class SignUp extends React.Component<any, any> {
  ...
  render() {
        ...
        <div id="slider-container" className="swiper-container">
          ...
          <div className="pagination">
            <div className={`bg-hr member2`}
                 style={{left:`${55}px`,width:`${(window.innerWidth*0.74 - 140) +10}px`}}></div>
            <div className={`bg-hr member3`}
                 style={{left:`${window.innerWidth*0.74*0.5 + 10}px`,width:`${(window.innerWidth*0.74 - 140)/2 + 10}px`}}></div>
    ...
    const renderMenu = (showMember = {}) => {
      ...
      return (
        <span>报名{name}（¥{showMember.fee?numeral(showMember.fee).format('0.00'):null}/{showMember.id === 4 ? '半年' : '年'}）</span>
      )
    }
